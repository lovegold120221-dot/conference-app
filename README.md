...
The agent expects these env vars (set via `--secrets-file` or LiveKit Cloud dashboard):
- `LIVEKIT_URL`, `LIVEKIT_API_KEY`, `LIVEKIT_API_SECRET`
- `GEMINI_API_KEY`

The agent requires a persistent process — it cannot run on Vercel.

**Frontend** — one-click deploy to Vercel:

[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/YOUR_REPO/gemini-live-translate&env=LIVEKIT_URL,LIVEKIT_API_KEY,LIVEKIT_API_SECRET)
...
The frontend needs only those three LiveKit env vars. The `GEMINI_API_KEY` stays on the agent side — translation happens server-side in the agent, not in the browser.

> **Architecture note:** This is a hybrid deploy. The Next.js frontend + API routes run on Vercel (serverless). The Python translator agent runs as a persistent LiveKit Cloud Agent. The agent is dispatched automatically when a room is created via the token endpoint — Vercel doesn't need to know about it.
